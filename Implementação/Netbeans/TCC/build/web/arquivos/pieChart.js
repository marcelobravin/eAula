...
C.alias=function(K,M,L){K=this.prototype[K];
...
(function(B){for(var A in B){Native.typize(B[A],A.toLowerCase());

}})({Boolean:Boolean,Native:Native,Object:Object});
(function(B){for(var A in B){new Native({name:A,initialize:B[A],protect:true});
}})({String:String,Function:Function,Number:Number,Array:Array,RegExp:RegExp,Date:Date});

(function(C,B){for(var D=0,A=B.length;
D<A;
D++){Native.genericize(C,B[D],true);
...
B<A;
B++){if($defined(arguments[B])){return arguments[B];
...
function $try(B,D,A){try{return B.apply(D,$splat(A));
...
}for(var B in A){if(!this[B]){this[B]=A[B];
}}return this;
}});
...
function $A(C){if($type(C)=="collection"){var D=[];
...
var Window=new Native({name:"Window",legacy:window.Window,initialize:function(A){if(!A.Element){A.Element=$empty;
...
var PieChart = new Class({
	...
	insertElements: function(el){
		...
		new Asset.image('arquivos/spacer.gif', {
			alt: '',
